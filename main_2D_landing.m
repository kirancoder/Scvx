...
% 1. Load parameters into the workspace
load_parameters;
...
% 4. Setup Integrator and Problem wrapper
integrator = Discretization(m, K, sigma);
...
for it = 1:max_iterations
    ...
    % Pass updated matrices to the CVX problem wrapper
    ...
    while true
        ...
        % Integrate nonlinear dynamics to check linearization accuracy
...
figure('Name', 'Rocket Landing 2D Trajectory', 'Position', [100, 100, 600, 800]);
plot(final_X(1, :), final_X(2, :), 'Color', [0.8 0.8 0.8], 'LineWidth', 2);
hold on;
grid on;
xlabel('X (East) [m]');
ylabel('Y (Up) [m]');
title(sprintf('Rocket Landing 2D - Iteration %d', length(all_X)-1));
