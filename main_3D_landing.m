...
% 2. Initilize 3D Model
m = RocketLanding3D(K);
m.nondimensionalize();

% 3. Initilize trajectory
...
% 4. Setup Integrator and Problem wraper
...
% 5. Start Successive Convexfication Loop
...
figure('Name', 'Rocket Landing 3D Trajectory', 'Position', [100, 100, 800, 800]);
plot3(final_X(2, :), final_X(3, :), final_X(4, :), 'Color', [0.7 0.7 0.7], 'LineWidth', 2);
hold on; grid on; view(3);
xlabel('X (East) [m]'); ylabel('Y (North) [m]'); zlabel('Z (Up) [m]');
title(sprintf('Rocket Landing 3D - Iteration %d', length(all_X)-1));
